...
func instance_level_idx(idx : int) -> void:
	...
	level_template_node.connect("player_win", self, "_on_player_win_level")
...
func _on_player_win_level():
	$ScoreContainer/Score/Label.text = $ScoreContainer/Score/Label.text.replace("<score>", String(self.score).pad_zeros(3))
